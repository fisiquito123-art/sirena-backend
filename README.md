...
## 0. Base de datos permanente (Turso) — importante para Render gratis

En el plan gratuito de Render, el disco **no es permanente**: cada vez que el
servidor se reinicia (se duerme por falta de uso, o subes un cambio de
código), el archivo `reportes.db` se borra y todo empieza de cero. Para
evitar eso, este proyecto puede conectarse a **Turso**, una base de datos en
la nube gratuita y permanente, compatible con SQLite (casi no cambia nada
del código).

**Si no configuras nada, sigue funcionando igual que antes** (guarda en un
archivo local `reportes.db`) — perfecto para probar en tu computadora. Pero
para la versión pública en Render, sí conviene configurar Turso:

1. Entra a https://turso.tech y crea una cuenta gratis.
2. Crea una base de datos nueva (el plan gratis alcanza de sobra para esto).
3. Turso te da dos datos: una **URL de la base de datos** (empieza con
   `libsql://...`) y un **Auth Token**.
4. En Render, ve a tu servicio → **Environment** → agrega dos variables:
   - `TURSO_DATABASE_URL` = la URL que te dio Turso
   - `TURSO_AUTH_TOKEN` = el token que te dio Turso
5. Render se re-despliega solo. Desde ese momento, los reportes quedan
   guardados para siempre, aunque el servidor se duerma o reinicie.
